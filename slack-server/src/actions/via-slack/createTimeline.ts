...
export default async function handleMessageEvent(event: any) {
  ...
  try {
    const answer: AxiosResponse<any, any> = await axios.get(`${IMS_SERVER_ROUTING}incident/${event.channel}/channelId`, { headers });
    const answerData: AxiosResponse<any, any> = await axios.get(`${IMS_SERVER_ROUTING}incident/${event.channel}/channelId`, { headers });
    if (answerData.data) {
